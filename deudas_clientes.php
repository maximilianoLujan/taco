...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('deudas-clientes-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));// no necesitamos "ver" siempre va en pesos
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$valores[$fecha] = calcular_deudas_clientes($sucursal,$fecha,$periodo);
		}
		if($ai == 'A.I Si'){
...
//			$valores = array_reverse( $valores_i, true );
		...
		$colores_por_promedio = colores_inversos_por_promedio($valores,$fechas,$periodo,0);//el cero es para evitar el ajuste en el ultimo periodo
		// Dataset definition
		$myData = new pData();
