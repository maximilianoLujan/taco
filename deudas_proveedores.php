...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('deudas-proveedores-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));// no necesitamos "ver" siempre va en pesos
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$valores[$fecha] = calcular_deudas_proveedores($fecha,$periodo);
		}
		if($ai == 'A.I Si'){
