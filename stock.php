...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('stock-'.$ver.'-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$total_fecha = calcular_stock($sucursal,$fecha,$periodo);
			if($ver=='Ver $'){
				$valores[$fecha] = round($total_fecha['monto']);
			}else{
				$valores[$fecha] = $total_fecha['unidades'];
			}
		}
...
//			$valores = array_reverse( $valores_i, true );
		...
		$colores_por_promedio = colores_por_promedio($valores,$fechas,$periodo,0);//el cero es para evitar el ajuste en el ultimo periodo
		// Dataset definition
		$myData = new pData();
