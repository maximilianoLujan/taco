...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('ingresos-egresos-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));// no necesitamos "ver" siempre va en pesos
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$valores[$fecha] = ingresos_egresos_principal($fecha,$periodo);
		}
		if($ai == 'A.I Si'){
