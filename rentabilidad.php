...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('rentabilidad-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));// no necesitamos "ver" siempre va en pesos
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$valores[$fecha] = calcular_rentabilidad($sucursal,$fecha,$periodo);
		}
		if($ai == 'A.I Si'){
