...
if( @$_GET ){
	...
	$ver = $_GET['ver'];
	if( $ver == 'Ver $/U' ) $ver = 'Ver U';// parche para evitar cachear dos veces las imagenes que son iguales (Ver U y Ver $/U)
...
//	$fechas = array_reverse(fechas_a_mostrar($periodo,0,12));
	...
	$cacheKey = urlencode('ventas-'.$ver.'-'.$sucursal.'-'.$ai.'-'.$periodo.'-'.implode('-',$fechas));
	...
	if(file_exists("./cache/".$ChartHash.".png")){
		...
		foreach($fechas as $fecha){
			...
			$total_fecha = calcular_ventas($sucursal,$fecha,$periodo);
			if($ver=='Ver $'){
				$valores[$fecha] = round($total_fecha['monto']);
			}else{
				$valores[$fecha] = $total_fecha['cantidad'];
			}
		}
		if($ai == 'A.I Si' && $ver=='Ver $'){
...
//			$valores = array_reverse( $valores_i, true );
		...
		$colores_por_promedio = colores_por_promedio($valores,$fechas,$periodo,1);// para equiparar el ultimo periodo incompleto con los otros completos
		// Dataset definition
		$myData = new pData();
